...
describe('SubmissionMaterialsService', () => {
  ...
  const mockDocuments = [
    {
      id: 'doc-1',
      rfpId: 'rfp-123',
      extractedText: 'This document contains technical requirements for the project including security compliance, performance metrics, and delivery timelines.',
    },
  ]
  ...
  beforeEach(() => {
    ...
    ;(storage.getCompanyProfile as jest.Mock).mockResolvedValue(mockCompanyProfile)
    ;(storage.getAllCompanyProfiles as jest.Mock).mockResolvedValue([mockCompanyProfile])
    ;(storage.getProposalByRFP as jest.Mock).mockResolvedValue(null)
    ;(storage.createProposal as jest.Mock).mockResolvedValue({ id: 'proposal-123' })
  })
  ...
  describe('generateSubmissionMaterials', () => {
    it('should successfully generate submission materials using Mastra agents', async () => {
      ...
      ;(proposalManager.generateVNext as jest.Mock).mockResolvedValue(mockProposalResponse)
      ;(contentGenerator.generateVNext as jest.Mock).mockResolvedValue(mockDetailedResponse)
      ;(complianceChecker.generateVNext as jest.Mock).mockResolvedValue({ success: true })
    ...
    it('should fall back to OpenAI when Mastra agents fail', async () => {
      ...
      ;(proposalManager.generateVNext as jest.Mock).mockRejectedValue(new Error('Agent failed'))
    ...
    it('should generate pricing tables correctly', async () => {
      ...
      ;(proposalManager.generateVNext as jest.Mock).mockResolvedValue({ text: 'Mock proposal' })
      ;(contentGenerator.generateVNext as jest.Mock).mockResolvedValue({ text: 'Mock content' })
        ...
        choices: [{
          message: {
            content: JSON.stringify({
              items: [
                { description: 'Project Management', quantity: 100, unitPrice: 125, total: 12500 },
                { description: 'Development Services', quantity: 200, unitPrice: 100, total: 20000 },
              ]
            })
          }
        }]
      ...
      const request = {
        ...
        pricingData: {
          items: [
            { name: 'PM Services', category: 'Labor', unitPrice: 125, unit: 'hour', margin: 40 }
          ],
          defaultMargin: 40,
          laborRate: 100,
        }
    ...
    it('should update existing proposals instead of creating new ones', async () => {
      ...
      ;(storage.getProposalByRFP as jest.Mock).mockResolvedValue(existingProposal)
      ;(proposalManager.generateVNext as jest.Mock).mockResolvedValue({ text: 'Mock proposal' })
      ;(contentGenerator.generateVNext as jest.Mock).mockResolvedValue({ text: 'Mock content' })
  ...
  describe('formatTechnicalProposal', () => {
    it('should format technical proposal correctly', () => {
      ...
      const formatTechnicalProposal = (service as any).formatTechnicalProposal.bind(service)
  ...
  describe('formatPricingSchedule', () => {
    it('should format pricing schedule correctly', () => {
      const pricingData = {
        lineItems: [
          { description: 'Project Management', quantity: 100, unitPrice: 125, total: 12500 },
          { description: 'Development', quantity: 200, unitPrice: 100, total: 20000 },
        ],
        summary: {
          subtotal: 32500,
          tax: 2681.25,
      ...
      const formatPricingSchedule = (service as any).formatPricingSchedule.bind(service)
