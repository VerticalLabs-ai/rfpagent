...
export class DatabaseStorage implements IStorage {
  ...
  async updatePortal(id: string, updates: Partial<Portal>): Promise<PublicPortal> {
  ...
  async getAllRFPs(filters?: {
    ...
    const mainQueryWithConditions = conditions.length > 0
    ...
    const orderedQuery = mainQueryWithConditions.orderBy(desc(rfps.discoveredAt))

    const finalMainQuery = filters?.limit && filters?.offset
    ...
    const [rfpData, totalData] = await Promise.all([finalMainQuery, countQueryWithConditions])
  ...
  async getSubmissionByProposal(proposalId: string): Promise<Submission | undefined> {
    const [submission] = await db
      .select()
      .from(submissions)
