...
export class AnalysisTestRunner {
  ...
  async runAnalysisTests(): Promise<TestResult[]> {
    ...
    for (const testCase of testCases) {
      ...
      if (result.success) {
        ...
        console.log(`❌ ${testCase.name} - FAILED: ${result.errors.join(', ')}`);
      }
    }
  ...
  private async runSingleTest(testCase: AnalysisTestCase): Promise<TestResult> {
    ...
    try {
      ...
      const documents = await this.uploadTestDocuments(rfp.id, testCase.documents);
      ...
      const workflowResult = await this.analysisOrchestrator.executeAnalysisWorkflow({
        rfpId: rfp.id,
        sessionId,
        priority: 1
      });
      ...
      if (updatedRfp && this.validateDataPersistence(updatedRfp, updatedDocuments)) {
        result.results.dataValidated = true;
        console.log('✅ Data persistence validated');
        
        // Extract metrics
        result.metrics.requirementsExtracted = Array.isArray(updatedRfp.requirements) ? updatedRfp.requirements.length : 0;
        result.metrics.complianceItems = Array.isArray(updatedRfp.complianceItems) ? updatedRfp.complianceItems.length : 0;
        result.metrics.riskFlags = Array.isArray(updatedRfp.riskFlags) ? updatedRfp.riskFlags.length : 0;
      } else {
      ...
      if (updatedRfp?.complianceItems && Array.isArray(updatedRfp.complianceItems) && updatedRfp.complianceItems.length > 0) {
        result.results.complianceGenerated = true;
        console.log(`✅ Generated ${updatedRfp.complianceItems.length} compliance items`);
      } else {
        result.errors.push('No compliance items generated');
      }

      // Check overall success
      result.success = result.errors.length === 0 && Object.values(result.results).every(v => v);
   ...
   */
  private async createTestRFP(rfpData: AnalysisTestCase['rfpData']): Promise<RFP> {
    const rfp = await storage.createRFP({
      title: rfpData.title,
      description: rfpData.description,
      deadline: rfpData.deadline || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days from now
  ...
  private validateDataPersistence(rfp: RFP, documents: Document[]): boolean {
    ...
    if (!Array.isArray(rfp.complianceItems) || rfp.complianceItems.length === 0) {
    ...
    const processedDocuments = documents.filter(doc => 
      doc.extractedText && doc.parsedData
    );
    ...
    const documentsWithAnalysis = documents.filter(doc => 
      doc.parsedData && typeof doc.parsedData === 'object'
    );
    ...
    return [
      {
        ...
        rfpData: {
          ...
          description: 'Request for proposal for custom software development services including web applications and mobile apps.',
...
- NIST cybersecurity framework compliance`,
      ...
      },
      {
        ...
        rfpData: {
          ...
          description: 'Comprehensive modernization of legacy IT systems including cloud migration, security enhancements, and system integration.',
...
Management Approach: 10%`,
  ...
  async runSmokeTest(): Promise<boolean> {
    ...
    try {
      ...
      await analysisProgressTracker.completeWorkflow(workflowId, { test: 'completed' });
