...
export class FederalRfpSearchService {
   ...
   */
  async searchUSASpending(criteria: FederalSearchCriteria): Promise<FederalRfpResult[]> {
    try {
      const searchParams = this.buildUSASpendingQuery(criteria);

      const response = await axios.post(`${this.USASPENDING_BASE_URL}/search/spending_by_award/`, {
   ...
   */
  async searchSAMGov(criteria: FederalSearchCriteria): Promise<FederalRfpResult[]> {
    try {
   ...
   */
  async searchFPDS(criteria: FederalSearchCriteria): Promise<FederalRfpResult[]> {
    try {
   ...
   */
  async searchFederalOpportunities(criteria: FederalSearchCriteria): Promise<FederalRfpResult[]> {
    const results: FederalRfpResult[] = [];

    try {
      // Search multiple federal sources in parallel
      const [usaSpendingResults, samResults, fpdsResults] = await Promise.allSettled([
  ...
  private buildFPDSQuery(criteria: FederalSearchCriteria): string {
    ...
    if (criteria.location) {
      const stateCode = this.getStateCode(criteria.location);
      if (stateCode) {
        queryParts.push(`PRINCIPAL_PLACE_OF_PERFORMANCE_STATE_CODE:${stateCode}`);
      }
    }
  ...
  private parseUSASpendingResults(data: any, criteria: FederalSearchCriteria): FederalRfpResult[] {
    ...
    return data.results.map((item: any, index: number) => ({
      ...
      deadline: item['Period of Performance Current End Date'] || new Date(Date.now() + 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
  ...
  private parseSAMResults(data: any, criteria: FederalSearchCriteria): FederalRfpResult[] {
    ...
    return data.opportunitiesData.map((item: any, index: number) => ({
      ...
      description: item.description || item.synopsis || 'Federal procurement opportunity',
      agency: item.organizationName || item.department || 'Federal Agency',
      estimatedValue: parseFloat(item.awardCeiling) || parseFloat(item.awardFloor) || 100000,
      deadline: item.responseDeadLine || new Date(Date.now() + 45 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      source: 'SAM.gov',
  ...
  private parseFPDSResults(xmlData: string, criteria: FederalSearchCriteria): FederalRfpResult[] {
    ...
    try {
      ...
      return entries.slice(0, 10).map((entry, index) => {
        ...
        const summary = this.extractXMLValue(entry, 'summary') || 'Historical contract data';
        const link = this.extractXMLValue(entry, 'link.*href="([^"]*)"') || 'https://fpds.gov';
  ...
  private extractXMLValue(xml: string, pattern: string): string | null {
    const match = xml.match(new RegExp(`<${pattern}[^>]*>([^<]*)</${pattern.split(' ')[0]}>`, 'i'));
    return match ? match[1].trim() : null;
  }

  private deduplicateAndRank(results: FederalRfpResult[], criteria: FederalSearchCriteria): FederalRfpResult[] {
  ...
  private calculateRelevanceScore(result: FederalRfpResult, criteria: FederalSearchCriteria): number {
  ...
  private getStateCode(location: string): string | null {
    ...
    if (location.length === 2 && Object.values(stateCodes).includes(location.toUpperCase())) {
    ...
    const cityToState: { [key: string]: string } = {
      ...
      'philadelphia': 'PA', 'san diego': 'CA', 'san jose': 'CA', 'san francisco': 'CA',
      'seattle': 'WA', 'denver': 'CO', 'washington': 'DC', 'boston': 'MA',
      'atlanta': 'GA', 'miami': 'FL', 'orlando': 'FL', 'las vegas': 'NV'
    };
