...
export class DiscoveryManager {
  ...
  async executePortalAuthentication(workItem: WorkItem): Promise<any> {
    console.log(`🔐 Executing portal authentication for work item: ${workItem.id}`);
    ...
    try {
      ...
      scanManager.updateStep(scanId, 'authenticating', 10, `Authenticating with ${portal.name}`);
      scanManager.log(scanId, 'info', `Starting authentication for portal: ${portal.name}`);
      ...
      if (portal.loginRequired && portal.username && portal.password) {
        scanManager.log(scanId, 'info', 'Portal requires authentication, attempting login');
        
        // Use MastraScrapingService for intelligent authentication
        try {
          context.progress.authentication = 25;
          scanManager.updateStep(scanId, 'authenticating', 15, 'Performing intelligent authentication');
          ...
          scanManager.log(scanId, 'error', `Authentication failed: ${error.message}`);
          throw error;
        }
      } else {
        // Public portal - no authentication required
        scanManager.log(scanId, 'info', 'Portal is public, no authentication required');
        authResult = {
          success: true,
      ...
      scanManager.updateStep(scanId, 'authenticated', 20, 'Authentication completed successfully');
      
      return authResult;
      
    } catch (error) {
      console.error(`❌ Portal authentication failed for work item ${workItem.id}:`, error);
      ...
      scanManager.log(workItem.inputs.scanId, 'error', `Authentication failed: ${error.message}`);
      throw error;
    }
  }
  ...
  async executePortalScanning(workItem: WorkItem): Promise<any> {
    ...
    try {
      ...
      // Update scan progress
      scanManager.updateStep(scanId, 'navigating', 30, `Scanning ${context.portalName} for opportunities`);
      scanManager.log(scanId, 'info', `Starting intelligent content scanning`);

      // Assign specialist for scanning
      const scannerSpecialist = await this.assignSpecialistForTask('portal_scanning', ['portal_authentication', 'content_scanning']);
      if (!scannerSpecialist) {
        throw new Error('No available portal scanner specialist');
      }

      // Record specialist assignment
      await this.recordSpecialistAssignment(workItem.id, scannerSpecialist.agentId, 'portal_scanning');

      context.progress.scanning = 25;
      scanManager.updateStep(scanId, 'navigating', 40, 'Specialist assigned, beginning content analysis');
      ...
      try {
        ...
        scanManager.log(scanId, 'info', 'Performing intelligent portal analysis with Mastra AI agents');
        ...
        scanManager.log(scanId, 'info', `Scanning completed for ${context.portalName}`);
        ...
        scanManager.updateStep(scanId, 'extracting', 50, 'Content scanning completed, preparing for RFP extraction');
        ...
        scanManager.log(scanId, 'error', `Content scanning failed: ${error.message}`);
        throw error;
      }
      
    } catch (error) {
      console.error(`❌ Portal scanning failed for work item ${workItem.id}:`, error);
      ...
      scanManager.log(workItem.inputs.scanId, 'error', `Scanning failed: ${error.message}`);
      throw error;
    }
  }
  ...
  async executeRFPExtraction(workItem: WorkItem): Promise<any> {
    ...
    try {
      ...
      // Update scan progress
      scanManager.updateStep(scanId, 'extracting', 60, `Extracting RFPs from ${context.portalName}`);
      scanManager.log(scanId, 'info', `Starting RFP extraction and data parsing`);

      // Assign specialist for RFP extraction
      const extractorSpecialist = await this.assignSpecialistForTask('rfp_extraction', ['rfp_extraction', 'content_monitoring']);
      if (!extractorSpecialist) {
        throw new Error('No available RFP extractor specialist');
      }

      // Record specialist assignment
      await this.recordSpecialistAssignment(workItem.id, extractorSpecialist.agentId, 'rfp_extraction');

      context.progress.extraction = 25;
      scanManager.updateStep(scanId, 'extracting', 65, 'RFP extraction specialist assigned');

      try {
        // Use PortalMonitoringService for actual RFP discovery and persistence
        scanManager.log(scanId, 'info', 'Executing comprehensive RFP extraction with PortalMonitoringService');
        ...
        const extractionResult = await this.portalMonitoringService.scanPortalWithEvents(portalId, scanId);
        
        context.progress.extraction = 75;
        
        // Update context with discovered RFPs
        context.discoveredRFPs = extractionResult.discoveredRFPs.map(discoveredRfp => ({
        ...
        scanManager.log(scanId, 'info', `RFP extraction completed: ${extractionResult.discoveredRFPs.length} RFPs discovered`);
        ...
        scanManager.updateStep(scanId, 'parsing', 70, `RFP extraction completed: ${extractionResult.discoveredRFPs.length} RFPs found`);
        ...
        scanManager.log(scanId, 'error', `RFP extraction failed: ${error.message}`);
        throw error;
      }
      
    } catch (error) {
      console.error(`❌ RFP extraction failed for work item ${workItem.id}:`, error);
      ...
      scanManager.log(workItem.inputs.scanId, 'error', `RFP extraction failed: ${error.message}`);
      throw error;
    }
  }
  ...
  async executePortalMonitoring(workItem: WorkItem): Promise<any> {
    ...
    try {
      ...
      // Update scan progress
      scanManager.updateStep(scanId, 'saving', 80, `Finalizing monitoring for ${context.portalName}`);
      scanManager.log(scanId, 'info', `Setting up ongoing monitoring and generating final report`);

      // Assign specialist for monitoring
      const monitorSpecialist = await this.assignSpecialistForTask('portal_monitoring', ['content_monitoring', 'change_detection']);
      if (!monitorSpecialist) {
        throw new Error('No available portal monitor specialist');
      }

      // Record specialist assignment
      await this.recordSpecialistAssignment(workItem.id, monitorSpecialist.agentId, 'portal_monitoring');
      ...
      try {
        ...
        scanManager.updateStep(scanId, 'saving', 85, 'Configuring ongoing monitoring');
        ...
        const finalReport = {
          ...
          summary: {
            ...
            totalExecutionTime: Date.now() - new Date(scanManager.getActiveScan(scanId)?.startedAt || 0).getTime()
          },
        ...
        scanManager.log(scanId, 'info', `Portal monitoring setup completed: ${context.discoveredRFPs.length} RFPs will be tracked`);
        ...
        scanManager.updateStep(scanId, 'completed', 100, `Discovery workflow completed: ${context.discoveredRFPs.length} RFPs discovered`);
        ...
        scanManager.log(scanId, 'error', `Monitoring setup failed: ${error.message}`);
        throw error;
      }
      
    } catch (error) {
      console.error(`❌ Portal monitoring failed for work item ${workItem.id}:`, error);
      ...
      scanManager.log(workItem.inputs.scanId, 'error', `Portal monitoring failed: ${error.message}`);
      throw error;
    }
  }
   ...
   */
  private async assignSpecialistForTask(taskType: string, requiredCapabilities: string[]): Promise<AgentRegistry | null> {
    try {
      // Find available specialists with required capabilities
      const availableSpecialists = await agentRegistryService.findAgentsByCapability(requiredCapabilities, 'specialist');
      
      if (availableSpecialists.length === 0) {
        console.warn(`No specialists found with capabilities: ${requiredCapabilities.join(', ')}`);
        return null;
      }

      // Filter by status and availability
      const activeSpecialists = availableSpecialists.filter(agent => 
        agent.status === 'active' && 
        agent.parentAgentId === 'portal-manager'
      );
      ...
      await agentRegistryService.updateAgentStatus(selectedSpecialist.agentId, 'busy');
      
      console.log(`✅ Assigned specialist ${selectedSpecialist.agentId} for task: ${taskType}`);
      return selectedSpecialist;
      
    } catch (error) {
      console.error(`❌ Failed to assign specialist for task ${taskType}:`, error);
      return null;
    }
  }
   ...
   */
  private async recordSpecialistAssignment(workItemId: string, agentId: string, taskType: string): Promise<void> {
    ...
    const existingAssignments = this.specialistAssignments.get(workItemId) || [];
  ...
  async cleanupContext(sequenceId: string): Promise<void> {
    ...
    for (const [workItemId, assignments] of this.specialistAssignments.entries()) {
      if (assignments.some(a => a.workItemId.includes(sequenceId))) {
        for (const assignment of assignments) {
          await agentRegistryService.updateAgentStatus(assignment.agentId, 'active');
        }
