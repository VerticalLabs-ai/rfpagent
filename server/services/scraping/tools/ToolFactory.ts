...
import { z } from "zod";
import { stagehandActTool, stagehandExtractTool, stagehandAuthTool } from "../../../../src/mastra/tools";
...
export class ToolFactory {
  ...
  static createAuthenticationTool() {
    return createTool({
      ...
      execute: async ({ loginUrl, username, password, portalType, sessionId }) => {
    ...
    return createTool({
      ...
      execute: async ({ content, portalType, minOpportunities }) => {
        ...
        if (Array.isArray(content) && content.length < minOpportunities) {
          issues.push(`Found ${content.length} opportunities, expected at least ${minOpportunities}`);
          score -= 0.2;
        }
        ...
        if (portalType === 'bonfire_hub' && Array.isArray(content)) {
          ...
          if (!hasDeadlines) {
            suggestions.push("Consider extracting deadline information for Bonfire opportunities");
            score -= 0.1;
          }
        }
