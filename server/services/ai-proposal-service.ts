...
import { z } from 'zod';
import type { CompanyProfile, CompanyContact, CompanyCertification, CompanyInsurance } from '@shared/schema';
...
export class AIProposalService {
  ...
  async analyzeRFPDocument(rfpText: string): Promise<RFPAnalysisResult> {
    ...
    if (rfpText.length > 50000) {
      throw new Error('RFP text too long. Please provide a summary or key sections only.');
    }
...
Focus on information relevant to iByte Enterprises LLC, a construction/technology company that is woman-owned with certifications (WBENC, HUB, DBE, MBE, WBE).`;
    ...
    try {
      const response = await this.openaiClient.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert RFP analyst that extracts structured requirements from procurement documents. Return only valid JSON that matches the requested schema exactly.'
          },
          {
            role: 'user',
            content: prompt
      ...
      if (!validatedAnalysis.success) {
        console.error('Invalid analysis format from OpenAI:', validatedAnalysis.error);
      ...
      throw new Error(`Failed to analyze RFP: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async mapCompanyDataToRequirements(
    ...
    const relevantCertifications = this.selectRelevantCertifications(analysis, certifications);
    
    // AI-powered insurance matching
    const applicableInsurance = this.matchInsuranceRequirements(analysis, insurance);
    ...
    const businessClassifications = {
      ...
      nigp: companyProfile.nigpCodes ? companyProfile.nigpCodes.split(',').map(code => code.trim()) : [],
      categories: companyProfile.primaryBusinessCategory ? [companyProfile.primaryBusinessCategory] : [],
    };

    // Determine socio-economic qualifications
    const socioEconomicQualifications = {
      smallBusiness: true, // iByte is a small business
      womanOwned: relevantCertifications.some(cert => cert.certificationType.includes('WBENC') || cert.certificationType.includes('WBE')),
      minorityOwned: relevantCertifications.some(cert => cert.certificationType.includes('MBE')),
      veteranOwned: false, // Not applicable for iByte
      hubZone: relevantCertifications.some(cert => cert.certificationType.includes('HUB')),
    };
...
- Address: 11324 Four Points Dr Bldg II Ste 100, Austin, TX 78726`;
    ...
    try {
      const response = await this.openaiClient.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert proposal writer for government contracting. Generate professional, compliant proposal content that highlights company qualifications and addresses RFP requirements directly. Return only valid JSON that matches the requested schema.'
          },
          {
            role: 'user',
            content: prompt
      ...
      const validatedContent = GeneratedProposalContentSchema.safeParse(rawContent);
      
      if (!validatedContent.success) {
        console.error('Invalid proposal content format from OpenAI:', validatedContent.error);
  ...
  private selectRelevantCertifications(analysis: RFPAnalysisResult, certifications: CompanyCertification[]): CompanyCertification[] {
    const requiredCerts = analysis.requirements.certifications || [];
    const socioEconomicPrefs = analysis.requirements.socioEconomicPreferences || [];
    ...
    return certifications.filter(cert => {
      ...
      const socioEconomicMatch = socioEconomicPrefs.some(pref => {
        ...
        return (
          (prefLower.includes('woman') && cert.certificationType.includes('WBENC')) ||
          (prefLower.includes('minority') && cert.certificationType.includes('MBE')) ||
          (prefLower.includes('disadvantaged') && cert.certificationType.includes('DBE')) ||
  ...
  private matchInsuranceRequirements(analysis: RFPAnalysisResult, insurance: CompanyInsurance[]): CompanyInsurance[] {
  ...
  private assignContactRoles(analysis: RFPAnalysisResult, contacts: CompanyContact[]) {
    ...
    if (owner) {
      assignments.push({
        role: 'Business Owner',
        contact: owner,
        reason: 'Company owner for woman-owned business certification verification'
      });
    }

    // Assign decision makers based on required roles
    const decisionMakers = contacts.filter(c => c.contactType === 'decision_maker');
    decisionMakers.forEach(dm => {
      if (dm.decisionAreas && Array.isArray(dm.decisionAreas) && dm.decisionAreas.length > 0) {
...
CERTIFICATIONS:
${mapping.relevantCertifications.map(cert => `- ${cert.certificationType}: ${cert.certificationNumber} (Expires: ${cert.expirationDate})`).join('\n')}
...
ASSIGNED CONTACTS:
${mapping.assignedContacts.map(assignment => 
  `- ${assignment.role}: ${assignment.contact.name} (${assignment.contact.email})`
).join('\n')}

INSURANCE COVERAGE:
${mapping.applicableInsurance.map(ins => `- ${ins.insuranceType}: $${ins.coverageAmount} (${ins.carrier})`).join('\n')}
    `;
  }
