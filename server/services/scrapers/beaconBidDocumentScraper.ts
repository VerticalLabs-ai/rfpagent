...
import { downloadFile } from '../core/fileDownloadService';
    ...
    .array(
      z.object({
        ...
        category: z
          .string()
          .optional()
          .describe('Document category (solicitation, addendum, plans, etc.)'),
        fileType: z
          .string()
          .optional()
...
export class BeaconBidDocumentScraper {
  private objectStorage: ObjectStorageService;
  private sessionId: string;

  constructor() {
    this.objectStorage = new ObjectStorageService();
    this.sessionId = `beaconbid-${randomUUID()}`;
  }

  /**
   * Validate URL is from allowed BeaconBid domains
   */
  ...
  /**
   * Scrape and download all documents for a specific BeaconBid RFP
   */
  async scrapeRFPDocuments(
    ...
    console.log(
      `📄 Starting BeaconBid document scraping for RFP ${rfpId} from ${solicitationUrl}`
    );

    // Security: Validate URL is from allowed domains
    if (!this.validateUrl(solicitationUrl)) {
      throw new Error(
        `URL not from allowed BeaconBid domains: ${solicitationUrl}`
      );
    }

    try {
      // Step 1: Extract document information using performWebExtraction
      // BeaconBid is a JavaScript SPA, so Stagehand will wait for content to load
      console.log(
        `📊 Extracting document information from BeaconBid page (JavaScript-rendered)`
      );
      const extractionResult = await performWebExtraction(
        solicitationUrl,
        `Find all downloadable documents on this BeaconBid solicitation page. Look for:
        - PDF, DOC, DOCX, XLS, XLSX, DWG, ZIP files
        - Document links in the attachments or documents section
        - Document names, titles, and descriptions
        - File sizes if shown
        - Categorize documents (solicitation document, addendum, plans/drawings, specifications, bid forms, etc.)

        IMPORTANT: BeaconBid pages are JavaScript-rendered. Wait for the content to fully load.
        Look for sections labeled "Attachments", "Documents", "Files", or similar.
      ...
      if (documents.length === 0) {
        console.warn('⚠️ No documents found on the BeaconBid page');
        return [];
      }
      ...
      for (const docInfo of documents) {
        try {
          ...
          const tempDir = '/tmp/claude/rfp_downloads';
          try {
          ...
          const savedDoc = await storage.createDocument({
            ...
            parsedData: {
              ...
              fileSize: docInfo.size || undefined,
              needsFillOut: needsFillOut,
              extractionMethod: 'stagehand_beaconbid',
      ...
      console.error('❌ Error during BeaconBid document scraping:', error);
      throw error;
    } finally {
  ...
  private determineFileType(name: string, url: string): string {
    ...
    if (combined.includes('.dwg')) return 'dwg';
    if (combined.includes('.dxf')) return 'dxf';
    if (combined.includes('.zip')) return 'zip';
    return 'unknown';
  }

  /**
   * Categorize document by name and provided category
   */
  private categorizeDocument(name: string, providedCategory?: string): string {
    // Use provided category if available
    if (providedCategory) {
      return providedCategory.toLowerCase();
    }

    const lowerName = name.toLowerCase();

    // BeaconBid specific categories
    if (lowerName.includes('addendum') || lowerName.includes('addenda'))
      return 'addendum';
    ...
    if (lowerName.includes('bid form') || lowerName.includes('proposal form'))
      return 'bid_form';
    if (lowerName.includes('price') || lowerName.includes('cost'))
      return 'price_form';
    if (lowerName.includes('submittal')) return 'submittal';
    if (lowerName.includes('insurance')) return 'insurance';
    if (lowerName.includes('bond')) return 'bonding';
    if (lowerName.includes('reference')) return 'references';
    if (lowerName.includes('wage')) return 'wage_compliance';
    if (lowerName.includes('solicitation') || lowerName.includes('rfp'))
      return 'solicitation_document';
...
export const beaconBidDocumentScraper = new BeaconBidDocumentScraper();
