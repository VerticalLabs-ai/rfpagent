...
import { retryBackoffDlqService } from '../core/retryBackoffDlqService';
...
export class MastraWorkflowEngine {
  ...
  private initializePhaseStateMachine(): void {
    ...
    const phases: PhaseDefinition[] = [
      ...
      },
      {
        ...
        entryActions: [
          'initialize_proposal_generation',
          'allocate_proposal_agents',
        ],
    ...
    const transitions: Record<string, PhaseTransition[]> = {
      discovery: [
        ...
        },
        {
          ...
          to: 'cancelled',
          conditions: {
            or: [{ errorCount: { gte: 3 } }, { manualCancellation: true }],
          },
      ...
      analysis: [
        {
          ...
          actions: [
            'transition_to_proposal_generation',
      ...
      monitoring: [
        ...
        },
        {
          ...
          to: 'failed',
          conditions: {
            or: [{ criticalError: true }, { submissionRejected: true }],
          },
    ...
    console.log(
      `✅ Phase State Machine initialized with ${phases.length} phases and ${
   ...
   */
  async createWorkflowWithPhaseState(
    workflowId: string,
    initialPhase: string = 'discovery',
    context: Record<string, any> = {}
  ): Promise<WorkflowPhaseState> {
  ...
  async transitionWorkflowPhase(
    ...
    console.log(
      `🔄 Attempting phase transition: ${workflowId} ${currentPhase} → ${toPhase}`
    );
    ...
    if (!transition) {
      return {
        success: false,
        error: `No transition definition found for ${currentPhase} → ${toPhase}`,
      };
    }

    // Validate transition conditions
    const conditionCheck = await this.validateTransitionConditions(
      workflowId,
      transition,
      context || {}
    );
    if (!conditionCheck.valid) {
      ...
      return {
        success: false,
        error: `Transition conditions not met: ${conditionCheck.reasons?.join(
          ', '
        )}`,
      };
    }
    ...
    try {
      ...
      console.error(
        `❌ Phase transition failed: ${workflowId} ${currentPhase} → ${toPhase}:`,
        error
      );
  ...
  private async validateTransitionConditions(
    ...
    try {
      const valid = await this.evaluateConditions(
        transition.conditions,
        workflowContext
      );
      if (!valid) {
        reasons.push(
          `Conditions not satisfied for transition to ${transition.to}`
        );
      }
  ...
  private evaluateCondition(
    ...
    if (typeof condition === 'object' && condition !== null) {
      ...
      if (
        ...
        (contextValue === undefined || contextValue < condition.min)
      ) {
        return false;
      }
      if (
        condition.max !== undefined &&
        (contextValue === undefined || contextValue > condition.max)
      ) {
        return false;
      }
      ...
      if (
        ...
        (contextValue === undefined || contextValue >= condition.lt)
      ) {
        return false;
      }
      if (
        condition.lte !== undefined &&
        (contextValue === undefined || contextValue > condition.lte)
      ) {
        return false;
      }
      if (
        condition.gt !== undefined &&
        (contextValue === undefined || contextValue <= condition.gt)
      ) {
        return false;
      }
      if (
        condition.gte !== undefined &&
        (contextValue === undefined || contextValue < condition.gte)
      ) {
        return false;
      }
  ...
  private async executePhaseEntryActions(
    ...
    console.log(
      `🎯 Executing entry actions for phase '${phase}': ${phaseDefinition.entryActions.join(
        ', '
      )}`
    );
  ...
  private async executePhaseExitActions(
    ...
    console.log(
      `🏁 Executing exit actions for phase '${phase}': ${phaseDefinition.exitActions.join(
        ', '
      )}`
    );
  ...
  private async executePhaseAction(
    ...
    switch (action) {
      case 'initialize_discovery':
        console.log(
          `📋 Initializing discovery phase for workflow ${workflowId}`
        );
      ...
      case 'allocate_discovery_agents':
        console.log(
          `👥 Allocating discovery agents for workflow ${workflowId}`
        );
      ...
      case 'calculate_discovery_metrics':
        console.log(
          `📊 Calculating discovery metrics for workflow ${workflowId}`
        );
      ...
      case 'initialize_analysis':
        console.log(
          `🔍 Initializing analysis phase for workflow ${workflowId}`
        );
      ...
      case 'transition_to_analysis':
        console.log(
          `🔄 Transitioning to analysis phase for workflow ${workflowId}`
        );
      ...
      case 'cleanup_discovery_resources':
        console.log(
          `🧹 Cleaning up discovery resources for workflow ${workflowId}`
        );
      ...
      case 'log_cancellation_reason':
        console.log(
          `📝 Logging cancellation reason for workflow ${workflowId}`
        );
      ...
      default:
        console.log(
          `⚠️ Unknown phase action: ${action} for workflow ${workflowId}`
        );
    }
  }
  ...
  async resumeWorkflow(
    ...
    if (workflow.status !== 'suspended') {
      return {
        success: false,
        error: `Workflow is not suspended (current status: ${workflow.status})`,
      };
    }
  ...
  async cancelWorkflow(
    ...
    if (
      workflow.metadata.childWorkflowIds &&
      Array.isArray(workflow.metadata.childWorkflowIds)
    ) {
      console.log(
        `🔗 Cascading cancellation to ${workflow.metadata.childWorkflowIds.length} child workflows`
      );
    ...
    console.log(
      `❌ Workflow ${workflowId} cancelled by ${triggeredBy}${
        cascading ? ' (cascading)' : ''
      }`
    );
    return { success: true };
  ...
  async handleWorkItemFailure(
    ...
    success: boolean;
    action: 'retried' | 'moved_to_dlq' | 'permanent_failure';
    nextRetryAt?: Date;
    error?: string;
  }> {
    try {
      ...
      if (retryResult.shouldRetry && retryResult.nextRetryAt) {
        ...
        console.log(
          `🔄 Scheduled retry for work item ${workItemId} at ${retryResult.nextRetryAt}`
        );
        return {
          success: true,
          action: 'retried',
      ...
      console.error(
        `❌ Error handling work item failure ${workItemId}:`,
        processingError
      );
      return {
        success: false,
  ...
  async processWorkItemCompletion(
    ...
    try {
      ...
      if (!workItem) {
        console.error(
          `Work item ${workItemId} not found for completion processing`
        );
        return;
      }
      ...
      if (success) {
        ...
        await storage.updateWorkItem(workItemId, {
          ...
          actualDuration: workItem.startedAt
            ? Math.round(
                (Date.now() - new Date(workItem.startedAt).getTime()) /
                  (1000 * 60)
              )
            : undefined,
        });
        ...
        if (workItem.workflowId && workItem.isBlocking) {
          await this.handleBlockingWorkItemFailure(
            workItem.workflowId,
            workItemId,
            error
          );
      ...
      console.error(
        `❌ Error processing work item completion ${workItemId}:`,
        processingError
      );
    }
  }
   ...
   */
  private async handleBlockingWorkItemFailure(
    workflowId: string,
    workItemId: string,
    error?: string
  ): Promise<void> {
    ...
    console.log(
      `🚫 Blocking work item ${workItemId} failed, evaluating workflow ${workflowId} impact`
    );
    ...
    const isCritical = criticalFailures.some(critical =>
    ...
    if (isCritical) {
      ...
      workflow.metadata.blockedByFailures =
        workflow.metadata.blockedByFailures || [];
  ...
  private async updateWorkflowProgress(workflowId: string): Promise<void> {
    try {
      ...
      const failedItems = workItems.filter(
      ...
      console.log(
        `📊 Workflow ${workflowId} progress: ${completedItems}/${totalItems} (${progressPercentage}%)`
      );
      ...
      if (phaseDefinition) {
        ...
        if (phaseCompleted && workflow.canTransitionTo.length > 0) {
          console.log(
            `🎯 Phase ${workflow.currentPhase} completed for workflow ${workflowId}, checking for auto-transition`
          );
          ...
          if (nextPhase) {
            await this.transitionWorkflowPhase(
              ...
              {
                autoTransition: true,
                phaseProgress: workflow.metadata.workItemProgress,
              }
            );
          }
        }
      ...
      console.error(
        `❌ Error updating workflow progress for ${workflowId}:`,
        error
      );
    }
  }
  ...
  private determineNextPhase(
    ...
    const transitions = phaseDefinition.allowedTransitions.filter(
      t => t !== 'cancelled' // Don't auto-transition to cancelled
    );
  ...
  private initializeWorkflows() {
    ...
    console.log(
      '🔧 Mastra workflow engine initialized with specialized agents'
    );
  }
      ...
      case 'rfp_results':
        suggestions.push(
          ...
          },
          {
            ...
            description:
              'Find and analyze competitive landscape and historical bids',
            icon: 'TrendingUp',
            payload: {
        ...
        suggestions.push(
          {
            ...
            description:
              'Search for new RFP opportunities matching your criteria',
            icon: 'Search',
          ...
          },
          {
            ...
            description:
              'Review your current RFP pipeline and performance metrics',
            icon: 'BarChart3',
  ...
  private async initializeAgentCoordination(): Promise<void> {
    console.log(
      '🤝 Initializing enhanced agent coordination with memory persistence...'
    );
  ...
  async delegateToAgent(
    ...
    try {
      ...
      const coordinationRecord =
        await agentMemoryService.createCoordinationRequest({
      ...
      console.log(
        `📝 Agent processing completed, updating coordination status`
      );

      // Record the successful coordination
      await agentMemoryService.updateCoordinationStatus(
        coordinationId,
        'completed',
        result
      );
      console.log(
        `✅ Coordination status updated to completed for ID: ${coordinationId}`
      );
   ...
   */
  private async recordAgentExperience(
    agentId: string,
    experience: any
  ): Promise<void> {
    try {
   ...
   */
  async executeActionSuggestion(
    suggestion: ActionSuggestion,
    conversationId: string
  ): Promise<any> {
    switch (suggestion.action) {
      case 'workflow':
        return this.initiateWorkflow(
          suggestion.payload?.workflowId,
          suggestion.payload,
          conversationId
        );
  ...
  private generateAgentPrompt(agentId: string, context: any): string {
    switch (agentId) {
      case 'market-research-analyst':
        return `Please analyze the market conditions and competitive landscape for the following RFPs or context: ${JSON.stringify(
          context
        )}`;

      case 'compliance-specialist':
        return `Please review and analyze the compliance requirements for: ${JSON.stringify(
          context
        )}`;

      default:
        return `Please provide analysis and recommendations for: ${JSON.stringify(
          context
        )}`;
    }
  }
