...
export interface AgentKnowledgeEntry {
  id?: string;
  agentId: string;
  knowledgeType: 'rfp_pattern' | 'compliance_rule' | 'market_insight' | 'pricing_data' | 'strategy' | 'template';
  domain: string;
  title: string;
  description?: string;
  content: any;
...
export class AgentMemoryService {
  ...
  async storeMemory(entry: AgentMemoryEntry): Promise<any> {
    ...
    if (!memoryData.agentId || !memoryData.contextKey || !memoryData.title) {
      throw new Error('Missing required fields: agentId, contextKey, and title are required');
    }
  ...
  async getAgentMemories(agentId: string, memoryType?: string, limit?: number): Promise<any[]> {
  ...
  async updateMemory(memoryId: string, updates: Partial<AgentMemoryEntry>): Promise<any> {
  ...
  async storeKnowledge(entry: AgentKnowledgeEntry): Promise<any> {
    ...
    // Basic validation
    if (!knowledgeData.agentId || !knowledgeData.title || !knowledgeData.domain) {
      throw new Error('Missing required fields: agentId, title, and domain are required');
    }

    if (knowledgeData.confidenceScore < 0 || knowledgeData.confidenceScore > 1) {
      throw new Error('Confidence score must be between 0 and 1');
    }

    const validKnowledgeTypes = ['rfp_pattern', 'compliance_rule', 'market_insight', 'pricing_data', 'strategy', 'template'];
    if (!validKnowledgeTypes.includes(knowledgeData.knowledgeType)) {
      throw new Error(`Invalid knowledge type. Must be one of: ${validKnowledgeTypes.join(', ')}`);
    }
  ...
  async getAgentKnowledge(agentId: string, knowledgeType?: string, domain?: string, limit?: number): Promise<any[]> {
    return await storage.getAgentKnowledgeByAgent(agentId, knowledgeType, domain, limit);
  }

  async updateKnowledge(knowledgeId: string, updates: Partial<AgentKnowledgeEntry>): Promise<any> {
  ...
  async updateCoordinationStatus(coordinationId: string, status: string, response?: any): Promise<any> {
  ...
  async getRelevantMemories(agentId: string, context: any, limit: number = 10): Promise<any[]> {
    ...
    const scoredMemories = memories.map(memory => {
      ...
      if (context.tags && memory.tags) {
        const matchingTags = context.tags.filter((tag: string) => memory.tags.includes(tag));
  ...
  async getRelevantKnowledge(agentId: string, context: any, limit: number = 10): Promise<any[]> {
    // Get all knowledge for the agent
    const knowledge = await this.getAgentKnowledge(agentId, undefined, undefined, 100);
    ...
    const scoredKnowledge = knowledge.map(item => {
      ...
      if (context.knowledgeType && item.knowledgeType === context.knowledgeType) {
        relevanceScore += 35;
      }
      
      // Score based on tag matching
      if (context.tags && item.tags) {
        const matchingTags = context.tags.filter((tag: string) => item.tags.includes(tag));
        relevanceScore += matchingTags.length * 15;
      }
  ...
  async recordPerformanceMetric(agentId: string, metricType: string, value: number, context?: any): Promise<void> {
