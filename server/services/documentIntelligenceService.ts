...
export class DocumentIntelligenceService {
  ...
  async analyzeRFPDocuments(rfpId: string): Promise<DocumentAnalysisResult> {
    ...
    console.log(`Found ${documents.length} documents to analyze for RFP: ${rfp.title}`);
    ...
    const hasPricingFields = allFormFields.some(field => 
      field.category === 'pricing' || field.category === 'financial'
    );

    if (hasPricingFields) {
      console.log(`Pricing fields detected, researching competitive bids...`);
      competitiveBidAnalysis = await this.researchCompetitivePricing(rfp, allFormFields);
    }

    // Calculate estimated completion time
    const estimatedTime = this.calculateCompletionTime(allFormFields, humanOversightItems);
   ...
   */
  private async analyzeDocument(document: Document, rfp: RFP): Promise<{
    formFields: FormField[];
    humanOversightItems: HumanOversightItem[];
    instructions: string[];
  }> {
...
`;
    ...
    try {
      const response = await this.openaiClient.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert document analyst specializing in government RFP forms and requirements. Return only valid JSON.'
          },
          {
            role: 'user',
            content: prompt
      ...
      analysis.formFields = analysis.formFields.map((field: any, index: number) => ({
        ...field,
        id: field.id || `${document.id}_field_${index}`,
      }));

      analysis.humanOversightItems = analysis.humanOversightItems.map((item: any, index: number) => ({
        ...item,
   ...
   */
  private async researchCompetitivePricing(rfp: RFP, formFields: FormField[]): Promise<CompetitiveBidAnalysis> {
...
`;
    ...
    try {
      const response = await this.openaiClient.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a government contracting expert specializing in competitive pricing analysis. Return only valid JSON.'
          },
          {
            role: 'user',
            content: prompt
      ...
      const estimatedValue = rfp.estimatedValue ? parseFloat(rfp.estimatedValue) : 100000;
      return {
  ...
  private extractSearchTerms(rfp: RFP, formFields: FormField[]): string[] {
    ...
    const serviceKeywords = [
      'construction', 'technology', 'consulting', 'software', 'hardware',
      'maintenance', 'support', 'installation', 'development', 'design'
    ];
    ...
    formFields
      .filter(field => field.category === 'pricing' || field.category === 'financial')
      .forEach(field => {
        if (field.name.length < 50) { // Avoid very long field names
   ...
   */
  private calculateCompletionTime(formFields: FormField[], humanOversightItems: HumanOversightItem[]): string {
    let totalMinutes = 0;
   ...
   */
  async autoFillFormFields(rfpId: string, formFields: FormField[], companyProfileId?: string): Promise<FormField[]> {
    ...
    console.log(`Successfully auto-filled ${filledFields.filter(f => f.value).length} out of ${formFields.length} fields`);
    return filledFields;
  }
   ...
   */
  private async fillFormField(field: FormField, companyProfile: any): Promise<string | undefined> {
   ...
   */
  private fillCompanyInfoField(field: FormField, companyProfile: any): string | undefined {
    const lowerName = field.name.toLowerCase();
    const lowerDesc = field.description.toLowerCase();

    // Company name variations
    if (lowerName.includes('company') || lowerName.includes('business') || lowerName.includes('firm')) {
    ...
    // Business category
    if (lowerName.includes('category') || lowerName.includes('type') || lowerDesc.includes('business type')) {
      return companyProfile.primaryBusinessCategory || 'Construction/Technology Services';
    }
  ...
  private fillComplianceField(field: FormField): string | undefined {
    ...
    if (lowerName.includes('woman') || lowerName.includes('wbe') || lowerName.includes('wbenc')) {
      return 'Yes - WBENC Certified';
    }

    // HUB certification
    if (lowerName.includes('hub') || lowerDesc.includes('historically underutilized')) {
      return 'Yes - HUB Certified';
    }

    // DBE certification
    if (lowerName.includes('dbe') || lowerDesc.includes('disadvantaged business')) {
    ...
    // Small business
    if (lowerName.includes('small business') || lowerName.includes('small biz')) {
