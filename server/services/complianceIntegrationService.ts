...
import { complianceCheckerSpecialist } from './analysisSpecialists';
...
export class ComplianceIntegrationService {
   ...
   */
  async triggerComplianceAnalysisForDiscoveredRFP(rfpId: string): Promise<ComplianceAnalysisResult> {
    ...
    try {
      ...
      if (rfp.requirements && rfp.complianceItems && rfp.riskFlags) {
        console.log(`✅ RFP ${rfpId} already has compliance data, skipping analysis`);
      ...
      if (documents.length === 0) {
        console.log(`📄 No documents found for RFP ${rfpId}, performing basic compliance analysis`);
      ...
      if (documentsWithText.length === 0) {
        console.log(`📝 No extracted text found, starting full analysis workflow for RFP ${rfpId}`);
        return await this.startFullAnalysisWorkflow(rfp);
      }

      // Perform direct compliance analysis on existing text
      console.log(`⚡ Performing direct compliance analysis for RFP ${rfpId} with ${documentsWithText.length} documents`);
      return await this.performDirectComplianceAnalysis(rfp, documentsWithText);

    } catch (error) {
      console.error(`❌ Auto compliance analysis failed for RFP ${rfpId}:`, error);
      this.processingQueue.delete(rfpId);
      return {
        rfpId,
        success: false,
   ...
   */
  async batchProcessUnprocessedRFPs(limit: number = 50): Promise<ComplianceAnalysisResult[]> {
    console.log(`🔄 Starting batch processing of unprocessed RFPs (limit: ${limit})`);
    ...
    try {
      ...
      console.log(`📊 Found ${unprocessedRfps.length} unprocessed RFPs out of ${allRfps.length} total`);
      ...
      for (let i = 0; i < unprocessedRfps.length; i += concurrency) {
        ...
        console.log(`🔄 Processing batch ${Math.floor(i / concurrency) + 1} of ${Math.ceil(unprocessedRfps.length / concurrency)} (${batch.length} RFPs)`);
        ...
        batchResults.forEach((result, index) => {
          if (result.status === 'fulfilled') {
            ...
            console.error(`❌ Batch processing failed for RFP ${batch[index].id}:`, result.reason);
            results.push({
   ...
   */
  private async performBasicComplianceAnalysis(rfp: RFP): Promise<ComplianceAnalysisResult> {
    try {
      console.log(`📋 Performing basic compliance analysis for RFP: ${rfp.title}`);
      ...
      let complianceData;
      try {
        complianceData = JSON.parse(basicAnalysis);
      } catch (parseError) {
      ...
      console.error(`❌ Basic compliance analysis failed for RFP ${rfp.id}:`, error);
      return {
        rfpId: rfp.id,
        success: false,
   ...
   */
  private async performDirectComplianceAnalysis(rfp: RFP, documents: Document[]): Promise<ComplianceAnalysisResult> {
    try {
      console.log(`📋 Performing direct compliance analysis for RFP: ${rfp.title} with ${documents.length} documents`);
      ...
      if (!combinedText.trim()) {
        console.log(`⚠️ No meaningful text found, falling back to basic analysis`);
        return await this.performBasicComplianceAnalysis(rfp);
      }

      // Use AI service for comprehensive compliance analysis
      const aiAnalysis = await aiService.analyzeDocumentCompliance(combinedText, rfp);
      ...
      console.error(`❌ Direct compliance analysis failed for RFP ${rfp.id}:`, error);
      return {
        rfpId: rfp.id,
        success: false,
        error: error instanceof Error ? error.message : 'Direct analysis failed'
      };
    }
  }
   ...
   */
  private async startFullAnalysisWorkflow(rfp: RFP): Promise<ComplianceAnalysisResult> {
    try {
      ...
      return {
        ...
        metadata: {
          ...
          message: 'Full analysis workflow started - compliance data will be available when workflow completes'
        }
      };
      ...
      return {
        rfpId: rfp.id,
        success: false,
        error: error instanceof Error ? error.message : 'Full workflow start failed'
      };
    }
  }
  ...
  private formatAndValidateComplianceData(rawData: any) {
    ...
    const complianceItems = Array.isArray(rawData.complianceItems) ? rawData.complianceItems : [];
    ...
    return {
      ...
      riskFlags: riskFlags.map((flag: any) => ({
        type: ['high', 'medium', 'low'].includes(flag.type) ? flag.type : 'medium',
  ...
  private generateDefaultComplianceData(rfp: RFP) {
    ...
    if (rfp.deadline) {
      const daysUntilDeadline = Math.ceil((new Date(rfp.deadline).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
  ...
  async onRFPDiscovered(rfpId: string): Promise<void> {
    ...
    setImmediate(async () => {
      try {
        ...
        console.error(`❌ Auto compliance analysis hook failed for RFP ${rfpId}:`, error);
      }
    });
  }
