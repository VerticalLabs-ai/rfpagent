...
export class PersistentMemoryEngine {
  ...
  async initializeSessionContext(sessionRequest: {
    ...
    try {
      ...
      const relevantContext = await this.retrieveRelevantContext(sessionRequest)
      ...
      console.log(`✅ Session context initialized: ${sessionContext.sessionId}`)
  ...
  async finalizeSession(sessionId: string): Promise<void> {
    try {
      ...
      const sessionLearnings = await this.extractSessionLearnings(
        sessionContext
      )
  ...
  private async compressMemories(
    ...
    try {
      ...
      const compressedCount = await this.mergeSimilarMemories(
        memoriesToCompress
      )
  ...
  private calculateGraphStrength(
    ...
    const totalConnections = edges.reduce((sum, edge) => sum + edge.strength, 0)
  ...
  private extractPatternContent(memories: any[]): any {
    ...
    return {
      ...
      timeRange: {
        start: Math.min(
          ...memories.map((m) => new Date(m.createdAt).getTime())
        ),
