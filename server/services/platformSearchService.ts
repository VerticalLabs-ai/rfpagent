...
export class PlatformSearchService {
   ...
   */
  async searchBidNet(criteria: PlatformSearchCriteria): Promise<PlatformRfpResult[]> {
    try {
      const apiKey = process.env.BIDNET_API_KEY;
      if (!apiKey) {
        console.warn('BidNet API key not configured, using web scraping fallback');
        return this.searchBidNetWebScraping(criteria);
      }

      // BidNet API integration (if available)
      const response = await axios.get('https://www.bidnet.com/api/opportunities', {
        params: {
          apikey: apiKey,
   ...
   */
  async searchDemandStar(criteria: PlatformSearchCriteria): Promise<PlatformRfpResult[]> {
    try {
   ...
   */
  async searchMERX(criteria: PlatformSearchCriteria): Promise<PlatformRfpResult[]> {
    try {
   ...
   */
  async searchGovWinIQ(criteria: PlatformSearchCriteria): Promise<PlatformRfpResult[]> {
    try {
      ...
      const response = await axios.post('https://api.govwin.com/search', {
   ...
   */
  async searchStateLocalProcurement(criteria: PlatformSearchCriteria): Promise<PlatformRfpResult[]> {
   ...
   */
  async searchAllPlatforms(criteria: PlatformSearchCriteria): Promise<PlatformRfpResult[]> {
    const results: PlatformRfpResult[] = [];

    try {
      // Search all platforms in parallel
      const [bidNetResults, demandStarResults, merxResults, govWinResults, stateResults] =
  ...
  private async searchBidNetWebScraping(criteria: PlatformSearchCriteria): Promise<PlatformRfpResult[]> {
  ...
  private async searchDemandStarWebScraping(criteria: PlatformSearchCriteria): Promise<PlatformRfpResult[]> {
  ...
  private parseBidNetResults(data: any, criteria: PlatformSearchCriteria): PlatformRfpResult[] {
    ...
    return data.opportunities.map((item: any, index: number) => ({
      ...
      description: item.description || item.summary || 'Government procurement opportunity',
      agency: item.agency || item.organization || 'Government Agency',
      estimatedValue: parseFloat(item.estimatedValue) || parseFloat(item.budgetRange?.max) || 200000,
      deadline: item.deadline || item.submissionDeadline || new Date(Date.now() + 45 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      source: 'BidNet',
  ...
  private parseMERXResults(data: any, criteria: PlatformSearchCriteria): PlatformRfpResult[] {
    ...
    return data.tenders.map((item: any, index: number) => ({
      ...
      deadline: item.closingDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      source: 'MERX',
      sourceUrl: item.detailUrl || `https://www.merx.com/tender/${item.tenderId}`,
  ...
  private parseGovWinResults(data: any, criteria: PlatformSearchCriteria): PlatformRfpResult[] {
    if (!data?.results) return [];

    return data.results.map((item: any, index: number) => ({
      id: `govwin_${Date.now()}_${index}`,
      title: item.title || `${criteria.category} Opportunity`,
      description: item.synopsis || item.description || 'Federal opportunity from GovWin IQ',
      agency: item.agency || item.customerName || 'Federal Agency',
      estimatedValue: parseFloat(item.contractValue) || parseFloat(item.obligatedAmount) || 300000,
      deadline: item.proposalDueDate || item.awardDate || new Date(Date.now() + 45 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      source: 'GovWin IQ',
      sourceUrl: item.govwinUrl || `https://www.govwin.com/opportunity/${item.id}`,
      category: criteria.category || item.naicsDescription || 'Federal Opportunity',
      location: item.placeOfPerformance || criteria.location || 'Various Locations',
      confidence: 0.9
    }));
  }

  private async searchStateSite(site: any, criteria: PlatformSearchCriteria): Promise<PlatformRfpResult[]> {
  ...
  private getStateProcurementSites(location?: string): any[] {
    const sites = [
      { name: 'Texas SmartBuy', url: 'https://www.txsmartbuy.com', state: 'Texas', code: 'TX' },
      { name: 'California eProcure', url: 'https://www.caleprocure.ca.gov', state: 'California', code: 'CA' },
      { name: 'New York State Contract Reporter', url: 'https://www.nyscr.ny.gov', state: 'New York', code: 'NY' },
      { name: 'Florida VBS', url: 'https://www.myfloridamarketplace.com', state: 'Florida', code: 'FL' },
      { name: 'Illinois Procurement Gateway', url: 'https://www.buy4illinois.illinois.gov', state: 'Illinois', code: 'IL' }
    ];
  ...
  private deduplicateAndRank(results: PlatformRfpResult[], criteria: PlatformSearchCriteria): PlatformRfpResult[] {
    // Remove duplicates based on title similarity
    const uniqueResults = results.filter((result, index, self) =>
      index === self.findIndex(r =>
        this.calculateSimilarity(r.title, result.title) < 0.8
      )
    );
  ...
  private calculateRelevanceScore(result: PlatformRfpResult, criteria: PlatformSearchCriteria): number {
    let score = result.confidence;

    if (criteria.category && result.title.toLowerCase().includes(criteria.category.toLowerCase())) {
      score += 0.3;
    }

    if (criteria.location && result.location.toLowerCase().includes(criteria.location.toLowerCase())) {
      score += 0.2;
    }

    if (criteria.agency && result.agency.toLowerCase().includes(criteria.agency.toLowerCase())) {
      score += 0.3;
    }

    if (criteria.valueRange?.min && result.estimatedValue >= criteria.valueRange.min) {
      score += 0.1;
    }
