...
export class SharedMemoryProvider {
  ...
  public static getSharedMemory(): Memory {
    if (!SharedMemoryProvider.instance) {
      console.log('🧠 Creating shared Memory instance with agentMemoryService integration');
        ...
        provider: {
          ...
          store: async (sessionId: string, messages: any[]) => {
            ...
            if (!sessionId || sessionId.trim() === '' || sessionId === 'default') {
              ...
              console.warn(`⚠️ Using fallback session ID: ${safeSessionId}. Consider providing explicit sessionId to prevent potential cross-contamination.`);
            }
            
            // Filter out sensitive credentials before storing
            const sanitizedMessages = messages.map(msg => SharedMemoryProvider.sanitizeMessage(msg));
          ...
          retrieve: async (sessionId: string, limit?: number) => {
            ...
            if (!sessionId || sessionId.trim() === '' || sessionId === 'default') {
              ...
              console.warn(`⚠️ Empty sessionId provided for memory retrieval - returning empty messages to prevent cross-contamination`);
              return [];
            }
            
            const memories = await agentMemoryService.getAgentMemories(safeSessionId, 'working', limit || 50);
   ...
   */
  public static async storeAgentKnowledge(agentId: string, knowledge: {
    type: 'rfp_pattern' | 'compliance_rule' | 'market_insight' | 'pricing_data' | 'strategy' | 'template';
    domain: string;
    title: string;
    content: any;
    confidence?: number;
   ...
   */
  public static async getRelevantKnowledge(agentId: string, context: {
    domain?: string;
    keywords?: string[];
    type?: string;
  }): Promise<any[]> {
