...
const downloadDocumentsStep = createStep({
  ...
  outputSchema: z.object({
    downloadedFiles: z.array(z.object({
      originalUrl: z.string(),
      localPath: z.string(),
      fileName: z.string(),
      size: z.number()
    })),
    failedDownloads: z.array(z.object({
      url: z.string(),
      error: z.string()
    }))
  }),
  execute: async ({ inputData }) => {
    ...
    const downloadPromises = documents.map(async (doc) => {
      try {
        const fileName = path.basename(new URL(doc.url).pathname) || `document-${randomUUID()}.pdf`;
    ...
    console.log(`✅ Downloaded ${downloadedFiles.length} files, ${failedDownloads.length} failed`);
...
const uploadToStorageStep = createStep({
  ...
  outputSchema: z.object({
    ...
    failedUploads: z.array(z.object({
      fileName: z.string(),
      error: z.string()
    }))
  }),
