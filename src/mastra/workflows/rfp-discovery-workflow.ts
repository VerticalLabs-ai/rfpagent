...
import { pageNavigateTool, pageObserveTool, pageActTool, pageExtractTool, pageAuthTool } from '../tools';
...
const scrapePortalStep = createStep({
  ...
  execute: async ({ inputData: portal }) => {
    ...
    try {
      ...
      const opportunities = (extractionResult.data?.opportunities || []).map((opp: any) => ({
        ...opp,
        portalId: portal.id,
        confidence: 0.9
      }));
      
      console.log(`✅ Found ${opportunities.length} opportunities from ${portal.name}`);
...
const processDiscoveredRfpsStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    console.log(`💾 Processing ${allOpportunities.length} discovered opportunities`);
    
    for (const opportunity of allOpportunities) {
      try {
        // Check if RFP already exists
        const existingRfps = await storage.getRFPsByPortal(opportunity.portalId);
        const existing = existingRfps.find(rfp => 
          rfp.title.toLowerCase() === opportunity.title.toLowerCase()
        );
        ...
        if (!existing) {
          ...
          newRfps++;
        } else if (opportunity.deadline && existing.deadline !== opportunity.deadline) {
        ...
        console.error(`Failed to process opportunity: ${opportunity.title}`, error);
      }
    }
...
export const rfpDiscoveryWorkflow = createWorkflow({
  ...
  .then(createStep({
    ...
    execute: async ({ inputData, mastra }) => {
      ...
      console.log(`🚀 Preparing to scrape ${portalBatches.length} portals in parallel`);
