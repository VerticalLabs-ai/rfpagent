import { Paperclip, FileText, CheckSquare, FileQuestion, Loader2, FileDown, AlertTriangle } from "lucide-react";
...
export function RFPDocuments({
  ...
  const fillableDocuments = documents.filter(doc => (doc.parsedData as any)?.needsFillOut);
  const referenceDocuments = documents.filter(doc => !(doc.parsedData as any)?.needsFillOut);
