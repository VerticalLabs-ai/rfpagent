...
const getEventIcon = (type: string) => {
  switch (type) {
    case 'rfp_discovered': return FileText;
...
const getEventColor = (type: string) => {
  switch (type) {
    case 'rfp_discovered': return 'text-green-600 dark:text-green-400';
    case 'error': return 'text-red-600 dark:text-red-400';
    case 'scan_completed': return 'text-green-600 dark:text-green-400';
    case 'scan_failed': return 'text-red-600 dark:text-red-400';
    default: return 'text-gray-600 dark:text-gray-400';
  }
};

export function ScanProgress({ scanState, isConnected, error, onReconnect, onDisconnect }: ScanProgressProps) {
  if (!scanState) {
    return (
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center space-x-2">
            ...
            <span data-testid="text-portal-name">{scanState.portalName || 'Portal Scan'}</span>
          </CardTitle>
          <div className="flex items-center space-x-2">
            ...
            <Badge 
              variant={scanState.status === 'completed' ? 'default' : scanState.status === 'failed' ? 'destructive' : 'secondary'}
      ...
      <CardContent className="space-y-4">
        ...
        {error && (
          <div className="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-red-700 dark:text-red-300" data-testid="text-error">
                {error}
              </span>
            </div>
            <Button 
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            <span data-testid="text-current-step">{stepLabels[scanState.currentStep.step]}</span>
            <span data-testid="text-progress-percent">{scanState.currentStep.progress}%</span>
          </div>
          <Progress 
          ...
          />
          <p className="text-sm text-gray-600 dark:text-gray-400" data-testid="text-step-message">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div className="space-y-1">
            <div className="text-2xl font-bold text-green-600 dark:text-green-400" data-testid="text-rfps-count">
              {scanState.rfpsDiscovered.length}
            </div>
            <div className="text-xs text-gray-500">RFPs Found</div>
          </div>
          <div className="space-y-1">
            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400" data-testid="text-events-count">
        ...
        <div className="space-y-2">
          ...
          <ScrollArea className="h-32 w-full rounded border p-2">
            <div className="space-y-2" data-testid="feed-activity">
              {scanState.events.slice(-10).reverse().map((event, index) => {
                const EventIcon = getEventIcon(event.type);
                const eventColor = getEventColor(event.type);
                
                return (
                  <div key={index} className="flex items-start space-x-2 text-xs">
                    <EventIcon className={`h-3 w-3 mt-0.5 ${eventColor} flex-shrink-0`} />
