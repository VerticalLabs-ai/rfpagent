        ...
        <CardContent className="p-6 space-y-4 max-h-96 overflow-y-auto">
          {highRiskRfps.length > 0 ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs text-green-600 mt-1">All RFPs are compliant</p>
            </div>
          )}
        </CardContent>
      </Card>
...
function ActivityItem({ notification, index }: { notification: any; index: number }) {
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      default: return { icon: "fas fa-circle", color: "text-gray-500" };
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-foreground" data-testid={`activity-title-${notification.id}`}>
          {notification.title}
        </p>
        <p className="text-xs text-muted-foreground" data-testid={`activity-message-${notification.id}`}>
...
function ComplianceAlert({ rfp, index }: { rfp: any; index: number }) {
  const highRiskFlags = rfp.riskFlags?.filter((flag: any) => flag.type === "high") || [];
  ...
  const getRiskIcon = (category: string) => {
    ...
    if (category.toLowerCase().includes("insurance")) return "fas fa-shield-alt";
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-red-800 dark:text-red-200" data-testid={`compliance-alert-title-${rfp.id}`}>
          High Risk: {primaryFlag.category}
        </p>
        <p className="text-xs text-red-600 dark:text-red-300" data-testid={`compliance-alert-description-${rfp.id}`}>
          {primaryFlag.description}
        </p>
        <div className="flex items-center justify-between mt-2">
          <p className="text-xs text-red-500" data-testid={`compliance-alert-rfp-${rfp.id}`}>
            {rfp.title}
          </p>
          <p className="text-xs text-red-500" data-testid={`compliance-alert-deadline-${rfp.id}`}>
...
function ComplianceNotificationAlert({ notification, index }: { notification: any; index: number }) {
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-orange-800 dark:text-orange-200" data-testid={`compliance-notification-title-${notification.id}`}>
          {notification.title}
        </p>
        <p className="text-xs text-orange-600 dark:text-orange-300" data-testid={`compliance-notification-message-${notification.id}`}>
