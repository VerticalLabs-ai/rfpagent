...
export default function Compliance() {
  ...
  const rfpsWithCompliance =
    (Array.isArray(rfps) ? rfps : []).filter(
      (item: any) =>
    ...
    <div className="p-6">
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="high-risk" className="space-y-6">
          ...
          {rfpsWithCompliance.filter((item: any) =>
            hasHighRiskFlags(item.rfp.riskFlags)
          ).length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                All RFPs are currently compliant with low or manageable risk
                levels
              </p>
            </div>
          )}
...
function ComplianceMetricsCard({ rfps }: { rfps: any[] }) {
  ...
  const highRiskCount = rfps.filter(item =>
    hasHighRiskFlags(item.rfp.riskFlags)
  ).length;
  const mediumRiskCount = rfps.filter(item =>
    hasMediumRiskFlags(item.rfp.riskFlags)
  ).length;
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <CardTitle
              ...
              data-testid={`compliance-title-${item.rfp.id}`}
            >
              {item.rfp.title}
            </CardTitle>
            <p
              className="text-sm text-muted-foreground mt-1"
              data-testid={`compliance-agency-${item.rfp.id}`}
            >
              {item.rfp.agency}
            </p>
          </div>
          <Badge
            ...
            variant="outline"
            data-testid={`compliance-risk-${item.rfp.id}`}
          >
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div
                ...
                data-testid={`compliance-high-${item.rfp.id}`}
              >
                {highRiskCount}
              </div>
            ...
            <div>
              <div
                ...
                data-testid={`compliance-medium-${item.rfp.id}`}
              >
                {mediumRiskCount}
              </div>
            ...
            <div>
              <div
                ...
                data-testid={`compliance-low-${item.rfp.id}`}
              >
...
function HighRiskCard({ item }: { item: any }) {
  const highRiskFlags = (item.rfp.riskFlags || []).filter(
    (flag: any) => flag.type === 'high'
  );
  ...
  return (
    <Card
      ...
      data-testid={`high-risk-card-${item.rfp.id}`}
    >
      <CardHeader>
        <div className="flex items-start justify-between">
          <CardTitle
            className="text-lg leading-tight text-red-700 dark:text-red-300"
            data-testid={`high-risk-title-${item.rfp.id}`}
          >
            {item.rfp.title}
          </CardTitle>
          <Badge
            variant="destructive"
            data-testid={`high-risk-badge-${item.rfp.id}`}
          >
        ...
        <p
          ...
          data-testid={`high-risk-agency-${item.rfp.id}`}
        >
          {item.rfp.agency}
        </p>
      </CardHeader>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <p
                    ...
                    data-testid={`high-risk-category-${item.rfp.id}-${index}`}
                  >
                    {flag.category}
                  </p>
                  <p
                    className="text-xs text-red-600 dark:text-red-300"
                    data-testid={`high-risk-description-${item.rfp.id}-${index}`}
                  >
                    {flag.description}
                  </p>
                </div>
  ...
  return (
    <Card data-testid={`requirements-card-${item.rfp.id}`}>
      <CardHeader>
        ...
        <p
          className="text-sm text-muted-foreground"
          data-testid={`requirements-agency-${item.rfp.id}`}
        >
          {item.rfp.agency}
        </p>
      </CardHeader>
          ...
          <TabsContent value="requirements" className="space-y-3">
            {requirements.length > 0 ? (
              requirements.map((req: any, index: number) => (
                ...
                >
                  <i
                    className={`fas ${req.mandatory ? 'fa-exclamation-circle text-red-500' : 'fa-info-circle text-blue-500'} mt-1`}
                  ></i>
                  <div>
                    <p
                      className="text-sm font-medium"
                      data-testid={`requirement-type-${item?.rfp?.id || 'unknown'}-${index}`}
                    >
                      {req.type}
                    </p>
                    <p
                      className="text-xs text-muted-foreground"
                      data-testid={`requirement-description-${item?.rfp?.id || 'unknown'}-${index}`}
                    >
                      {req.description}
                    </p>
                    {req.mandatory && (
              ...
              complianceItems.map((item: any, index: number) => (
                ...
                >
                  ...
                  <div>
                    <p
                      className="text-sm font-medium"
                      data-testid={`mandatory-field-${item?.rfp?.id || 'unknown'}-${index}`}
                    >
                      {item.field}
                    </p>
                    <p
                      className="text-xs text-muted-foreground"
                      data-testid={`mandatory-description-${item?.rfp?.id || 'unknown'}-${index}`}
                    >
                      {item.description}
                    </p>
...
function calculateComplianceScore(riskFlags: any[]): number {
  ...
  const deduction =
    highRiskCount * 20 + mediumRiskCount * 10 + lowRiskCount * 5;
