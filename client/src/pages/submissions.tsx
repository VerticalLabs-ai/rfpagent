      ...
      {filteredRfps.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-foreground mb-2">
            {searchQuery || statusFilter !== "all" ? "No Matching Submissions" : "No Submissions Ready"}
          </h3>
          <p className="text-muted-foreground">
    ...
    >
      <CardHeader>
        <div className="flex items-start justify-between">
          <CardTitle className="text-lg leading-tight" data-testid={`submission-title-${item.rfp.id}`}>
            {item.rfp.title}
          </CardTitle>
          <Badge 
        ...
        {item.portal && (
          <p className="text-xs text-muted-foreground" data-testid={`submission-portal-${item.rfp.id}`}>
        ...
        <div className="space-y-3">
          {item.rfp.deadline && (
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium" data-testid={`submission-deadline-${item.rfp.id}`}>
          ...
          {item.rfp.estimatedValue && (
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium" data-testid={`submission-value-${item.rfp.id}`}>
          ...
          {item.proposal?.estimatedMargin && (
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-green-600" data-testid={`submission-margin-${item.rfp.id}`}>
                {item.proposal.estimatedMargin}%
              </span>
            </div>
          )}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          <label className="font-medium text-muted-foreground">RFP Title:</label>
          <p className="mt-1" data-testid="modal-rfp-title">{item.rfp.title}</p>
        </div>
        <div>
          <label className="font-medium text-muted-foreground">Agency:</label>
          <p className="mt-1" data-testid="modal-rfp-agency">{item.rfp.agency}</p>
        </div>
        <div>
          <label className="font-medium text-muted-foreground">Portal:</label>
          <p className="mt-1" data-testid="modal-rfp-portal">{item.portal?.name || "N/A"}</p>
        </div>
        <div>
      ...
      {item.rfp.deadline && (
        <div className="text-sm">
          ...
          <p className="mt-1" data-testid="modal-rfp-deadline">
            {new Date(item.rfp.deadline).toLocaleDateString()} at {new Date(item.rfp.deadline).toLocaleTimeString()}
          </p>
        </div>
      )}
      ...
      {item.proposal && (
        <div className="border-t pt-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              <label className="font-medium text-muted-foreground">Estimated Margin:</label>
              <p className="mt-1 text-green-600" data-testid="modal-proposal-margin">
                {item.proposal.estimatedMargin}%
              </p>
            </div>
          </div>
