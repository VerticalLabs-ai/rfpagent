...
function OverviewAnalytics({
  ...
  const submittedRfps =
    rfps?.filter((item: any) => item.rfp.status === 'submitted').length || 0;
  const pendingReview =
    rfps?.filter((item: any) => item.rfp.status === 'review').length || 0;
  const inProgress =
    rfps?.filter((item: any) =>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card data-testid="kpi-submission-rate">
          ...
          <CardContent>
            <div
              className="text-3xl font-bold text-foreground"
              data-testid="submission-rate-value"
            >
              {totalRfps > 0
                ? Math.round((submittedRfps / totalRfps) * 100)
                : 0}
              %
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card data-testid="performance-processing-time">
          ...
          <CardContent>
            <div
              className="text-3xl font-bold text-foreground"
              data-testid="processing-time-value"
            >
              {avgProcessingTime}h
            </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {portalActivity?.map((activity: any) => (
          <Card
            key={activity.portal.id}
            data-testid={`portal-analytics-${activity.portal.id}`}
          >
            <CardHeader>
              <CardTitle
                data-testid={`portal-analytics-name-${activity.portal.id}`}
              >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    ...
                    data-testid={`portal-rfp-count-${activity.portal.id}`}
                  >
                    {activity.rfpCount || 0}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span
                    ...
                    data-testid={`portal-success-rate-${activity.portal.id}`}
                  >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-sm font-medium ${
                      activity.portal.status === 'active'
                        ? 'text-green-600'
                        : activity.portal.status === 'maintenance'
                          ? 'text-yellow-600'
                          : 'text-red-600'
                    }`}
                    data-testid={`portal-status-${activity.portal.id}`}
                  >
                    {activity.portal.status.charAt(0).toUpperCase() +
                      activity.portal.status.slice(1)}
                  </span>
                </div>
        ...
        <CardContent>
          {monthlyData.length === 0 ? (
            <div className="py-12 text-center text-muted-foreground">
              ...
              <p className="text-sm">
                Start discovering RFPs to see monthly trends appear here
              </p>
            </div>
          ) : (
            <div className="space-y-4">
            ...
            {agencyData.slice(0, 5).map((agency: any, index: number) => (
              <div key={index} className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  <span
                    className="text-sm text-muted-foreground"
                    data-testid={`agency-count-${index}`}
                  >
                    {agency.count} RFPs
                  </span>
                  <div className="w-24">
...
function calculateAutomationEfficiency(rfps: any[]): number {
  if (!rfps || rfps.length === 0) return 0;
  const automatedTasks = rfps.filter(
    (item: any) => item.rfp.status !== 'discovered'
  ).length;
...
function calculateComplianceScore(rfps: any[]): number {
  ...
  const totalRiskPoints = rfps.reduce((acc: number, item: any) => {
    ...
    const highRisk =
      riskFlags.filter((flag: any) => flag.type === 'high').length * 20;
    const mediumRisk =
      riskFlags.filter((flag: any) => flag.type === 'medium').length * 10;
    const lowRisk =
      riskFlags.filter((flag: any) => flag.type === 'low').length * 5;
...
function calculateMonthlyTrends(rfps: any[]): any[] {
  ...
  rfps.forEach((item: any) => {
    const createdDate = new Date(item.rfp.createdAt);
  ...
  const maxCount = Math.max(...sortedMonths.map(m => m.count));
