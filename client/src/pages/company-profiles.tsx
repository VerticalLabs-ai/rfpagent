...
export default function CompanyProfiles() {
  ...
  const [expiringDialogType, setExpiringDialogType] = useState<
    'certifications' | 'insurance' | 'all'
  >('all');
  ...
  const decisionMakers = allContacts.filter(
    (contact: CompanyContact) =>
      contact.contactType === 'decision_maker' ||
  ...
  const companiesWithDecisionMakers =
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-green-600">
                  {profiles?.filter((p: CompanyProfile) => p.isActive).length ||
                    0}
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {companiesWithDecisionMakers}/{profiles?.length || 0}{' '}
                  companies covered
                </p>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {Array.from(
                    new Set(
                      profiles
                        ?.map((p: CompanyProfile) => p.registrationState)
                        .filter(Boolean)
                    )
                  )
                    .slice(0, 5)
                    .map(state => {
                      const count =
                        profiles?.filter(
                          (p: CompanyProfile) => p.registrationState === state
                        ).length || 0;
                      return (
                        <div
                          key={state}
