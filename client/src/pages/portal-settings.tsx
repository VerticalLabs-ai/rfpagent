...
export default function PortalSettings() {
  ...
  const handleUpdatePortal = (portalId: string) => (data: Partial<PortalFormData>) => {
  ...
  const getRFPCount = (portalId: string) => {
    return portalActivity?.filter((activity: any) => activity.portalId === portalId).length || 0;
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Portal Settings</h1>
          <p className="text-muted-foreground">Manage RFP portal connections and monitoring</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {portals?.map((portal: Portal) => (
                <PortalCard
                  ...
                  scanning={scanPortalMutation.isPending && activeScanPortalId === portal.id}
